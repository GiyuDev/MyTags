...
public enum XSound {
     ...
     */
    @Nullable
    public static CompletableFuture<Record> play(@NotNull Player player, @Nullable String sound, float volume, float pitch) {
        Objects.requireNonNull(player, "Cannot play sound to null player");
        return parse(player, player.getLocation(), sound, true, volume, pitch);
    }
     ...
     */
    @Nullable
    public static CompletableFuture<Record> play(@NotNull Location location, @Nullable String sound, float volume, float pitch) {
        return parse(null, location, sound, true, volume, pitch);
    }
     ...
     */
    @Nullable
    public static CompletableFuture<Record> parse(@Nullable Player player, @NotNull Location location, @Nullable String sound, boolean play, float volume, float pitch) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                if (split.length > 1) {
                	volume1 = Float.parseFloat(split[1]);
                    if (split.length > 2) pitch2 = Float.parseFloat(split[2]);
                }
            } catch (NumberFormatException ignored) {
            }

            Record record = new Record(type, player, location, volume1, pitch2, playAtLocation);
            if (play) record.play();
            return record;
        }).exceptionally((ex) -> {
